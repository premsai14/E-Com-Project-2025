...
//		add_action( 'woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_link_open', 9 );
    ...
    public function swatch_html( $attribute_type, $taxonomy, $terms, $available_options, $variations, $selected, $attributes_count, $size, $is_quick_view = false, $available_options_by_attr=array() ) {
        ...
        if (function_exists('etheme_get_option') && etheme_get_option('autoselect_filtered_attribute', 0)){
            $filtered_params = $this->get_filtered_params();

            foreach ($filtered_params as $key => $value) {
                if('pa_' . $value['filter']== sanitize_title( $taxonomy )){
                    ...
                    foreach( $terms as $term ) {
                        ...
                        if ( in_array( $term->slug, $available_options, true ) ) {
                            ...
                            if (!$image){
	                            ...
	                            $image = '<img src="'.apply_filters('et_sten_placeholder_url',ET_CORE_URL . 'packages/st-woo-swatches/public/images/placeholder.png' ) . '"/>';
                            }
	 ...
	 * Get Filte Params
	 */
	public function get_filtered_params() {
		$result = [];
